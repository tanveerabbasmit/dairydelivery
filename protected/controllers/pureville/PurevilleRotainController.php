...
class PurevilleRotainController extends Controller
{
    ...
    public function actionupdatesName(){
          ...
          $company_id = Yii::app()->user->getState('company_branch_id');
           ...
           $client_Object = Client::model()->findAllByAttributes(
            ...
            foreach($client_Object as $value){
              ...
              $fullAddress_name =  $value['house_no'].' '.$value['sub_no'].' '.$block_name.' '.$area_name;
              $client_id = $value['client_id']."<br>";

               $clientObject = Client::model()->findByPk(intval($client_id));

                $clientObject->fullname = $fullAddress_name ;
                $clientObject->address = $fullAddress_name ;
     ...
     public function actionRemoveSpace(){
          die("lock");
         $query_block = "Select * from client as c 
            where  c.company_branch_id ='14' ";

         $result = Yii::app()->db->createCommand($query_block)->queryAll();

           foreach ($result as $value){

               $client_id = $value['client_id'];
               $clientObject = Client::model()->findByPk(intval($client_id));
               $clientObject->fullname = ltrim($value['fullname']);
               $clientObject->save();
