...
class PaymentMasterController extends Controller
{
    ...
    public function actiongetCustomerLedger_dateRangeCustomerReport(){
        ...
        $company_id = Yii::app()->user->getState('company_branch_id');
        if($company_id=='19'){
            echo   getCustomerLedger_dateRangeCustomerReport_data::get_report_data_date_range_raej($data);
        }else{
            echo   getCustomerLedger_dateRangeCustomerReport_data::get_report_data_date_range($data);
    ...
    public function actiongetCustomerLedger_2(){


        $final_time_list = [];

        $one_object = [];
        $one_object['time'] =  date("h:i:s");
        $one_object['action_line'] =  '1294';
        $final_time_list[] = $one_object;
        ...
        foreach($clientResult as $value){
            ...
            $query_makePayment = "SELECT sum(pm.amount_paid) as amount_paid ,
    ...
    public function actiongetCustomerLedger(){
        ...
        $rec_mayment_list = rider_wise_recovery_report_data::get_payment_list($client_list,$year,$monthNum);
        $rec_delievry_list = rider_wise_recovery_report_data::total_delivery($client_list,$start_calculation,$endDate);

        $rec_payment_between_date_range_list = rider_wise_recovery_report_data::payment_between_date_range($client_list,$start_calculation,$endDate_p);

        $rec_currect_month_delivery = rider_wise_recovery_report_data::total_currect_month_delivery($client_list,$monthNum,$year);
        $rec_get_delivery_opening = rider_wise_recovery_report_data::get_delivery_opening($client_list,$start_calculation,$endDate_p);
        ...
        foreach($clientResult as $value){
           ...
           /* $query_makePayment = "SELECT sum(pm.amount_paid) as amount_paid ,u.full_name,r.fullname FROM payment_master as pm
            ...
            if(isset($rec_mayment_list[$client_id])){

                $Payment_result = $rec_mayment_list[$client_id];

                $totalMake_Payment =intval($Payment_result['amount_paid']);
                $payment_add_by_user = $Payment_result['full_name'].$Payment_result['fullname'] ;
          ...
          /*  if($company_id==1){
            ...
            $totaldeliverySum2 = 0;
            if(isset($rec_delievry_list[$client_id])){
                $totaldeliverySum2 = $rec_delievry_list[$client_id];
            }
           ...
           /* $queryDelivery2 ="Select IFNULL(sum(pm.amount_paid) , 0) as remainingAmount from payment_master as pm
            ...
            $totalRemaining2 =0;
            if(isset($rec_payment_between_date_range_list[$client_id])){
                $totalRemaining2 =$rec_payment_between_date_range_list[$client_id];
            }
           ...
           //
            $totaldeliverySum_current = 0;
            if(isset($rec_currect_month_delivery[$client_id])){
                $totaldeliverySum_current = $rec_currect_month_delivery[$client_id];
            }
            /* current month Delivery end*/
            /*===========================================opening Delivery Start====================================================*/
           ...
           /* if($company_id==1){
            ...
            $totaldeliverySum_opening = $deliveryResult_opening[0]['deliverySum'];*/
            $totaldeliverySum_opening =0;
            if(isset($rec_get_delivery_opening[$client_id])){
                $totaldeliverySum_opening =  $rec_get_delivery_opening[$client_id];
            }
