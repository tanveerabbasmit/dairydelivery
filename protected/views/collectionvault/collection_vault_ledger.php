...
<script src="<?php echo Yii::app()->theme->baseUrl; ?>/js/viewPageJs/reciveBillReport/collection_vault_ledger_grid.js"></script>
	...
	<div ng-controller="clintManagemaent" ng-init='init(<?php echo $data ?> , "<?php echo Yii::app()->createAbsoluteUrl('index.php/collectionvault/collection_vault_ledger_report_list_data'); ?>")'>
		...
		<table  id="payment_report" style="margin-top: 6px" >
            ...
            <tr ng-repeat="list in result">
                ...
                <td style="color:black;background-color: white" >{{$index+1}}</td>

                <td style="color:black;background-color: white" >{{list.date}} </td>

                <td style="color:black;background-color: white" >{{list.vendor_name}} </td>
                <td style="color:black;background-color: white" >{{list.reference_no}} </td>

                <td style="text-align: center;color:black;background-color: white" >

                   <span ng-show="$index!='0'"> {{list.payment | number :0}} </span>
                </td>
                <td style="text-align: center;color:black;background-color: white" >
                    
                    <span ng-show="$index!='0'"> {{list. recipt | number :0}}</span>

                </td>
                <td style="text-align: center;color:black;background-color: white" > {{list.balance | number}}</td>
            </tr>
            <tr>
                <th colspan="4"><a href="#">Total</a> </th>
                <th style="text-align: center"><a href="#">{{total_payment}}</a> </th>
                <th style="text-align: center"><a href="#">{{total_receipt}}</a> </th>
                <th></th>
            </tr>
