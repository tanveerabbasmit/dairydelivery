...
<script src="<?php echo Yii::app()->theme->baseUrl; ?>/js/viewPageJs/AddFollowUpClient/AddFollowUpClient-grid.js"></script>
...
<div id="testContainer" style="display: none" class="panel row" ng-app="clintManagemaent">
	<div ng-controller="clintManagemaent" ng-init='init(<?php echo $ColorTagList; ?> , <?php echo $dropReasonList; ?> , <?php echo ($todayDate);?> , <?php echo ($saleRepsList);?> ,<?php echo str_replace("'","&#39;",$reasonList ); ?>,<?php echo str_replace("'","&#39;",$blockList ); ?>,<?php echo str_replace("'","&#39;",$areaList ); ?>, <?php echo $allow_delete ?> ,<?php echo str_replace("'","&#39;",$CategoryList ); ?> ,<?php echo $companyID ?> ,  <?php echo str_replace("'","&#39;",$ClientOneObject ); ?> , <?php  echo $preferedTimeList ?>  , <?php echo $frequencyList ?> , <?php echo $productCount  ?> , <?php echo $clientCount ?> , <?php echo str_replace("'","&#39;",$zoneList ); ?> ,<?php echo str_replace("'","&#39;",$clientList ); ?>,"<?php echo Yii::app()->createAbsoluteUrl('client/saveNewClient'); ?>"
	, "<?php echo Yii::app()->createAbsoluteUrl('client/EditClient'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/delete'); ?>" , "<?php echo Yii::app()->createAbsoluteUrl('client/checkAlredyExistClient'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/searchClient'); ?>",
	 "<?php echo Yii::app()->createAbsoluteUrl('client/nextPagePagination_sample_addFoolowUp'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/getProductList'); ?>"
	 , "<?php echo Yii::app()->createAbsoluteUrl('client/PnextPagePagination'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/selectFrequencyForOrder'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/saveChangedayObjectQuantity'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/removeProductFormSchedual'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('client/getProductPriceList'); ?>", "<?php echo Yii::app()->createAbsoluteUrl('followUp'); ?>")'>
		...
		<div class="" style="margin: 10px">
			<div class="row" ng-show="mainPageSwitch">
                ...
                <div class="col-lg-2">
                    <select class="form-control input-sm" ng-model="sales_reps_id" ng-change="onchangeZoneAndStatus(0,0)">
                        <option value="0">All Sale Man</option>
                        <option ng-repeat="list in saleRepsList" value="{{list.sales_reps_id}}">{{list.name}}</option>
                    </select>
                </div>

				<div class="col-lg-2">
                    <select class="form-control input-sm" ng-model="serach_zone_id" ng-change="onchangeZoneAndStatus(0,0)">
						<option value="0">All Zone</option>
						<option ng-repeat="list in zoneList" value="{{list.zone_id}}">{{list.name}}</option>
					</select>
				</div>
				<div class="col-lg-2">
                    <select class="form-control input-sm" ng-model="serach_status_id" ng-change="onchangeZoneAndStatus(0,0)">
		    	...
		    	<table id="customers">
		      		<thead>
		        		<tr style="background-color: #F0F8FF">
				          	<th><a href="#">ID</a></th>
				          	<th>
                                <a href="#" ng-click="dateSort(1)">
                                    Drop Date
                                    <span ng-show="!sortType_date && default_sort_date" class="fa fa-caret-down ng-hide"></span>
                                    <span ng-show="sortType_date && default_sort_date" class="fa fa-caret-up"></span>
                                </a>
				          	...
				          	<th><a href="#">Last Follow up Date </a></th>
				          	<th><a href="#">Last Follow up Remarks </a></th>
				            <!--	<th width="100px"><a href="#">Last Login</a> </th>-->
                            <!--	<th><a href="">Status</a> </th>-->
							<th><a href="">Sale Reps</a> </th>
							<th>
                                <a href="#" ng-click="dateSort(2)">
                                    Tag
                                    <span ng-show="!sortType_tag && default_sort_tag" class="fa fa-caret-down ng-hide"></span>
                                    <span ng-show="sortType_tag && default_sort_tag" class="fa fa-caret-up"></span>
                                </a>
                              <!--  <a href="">Tag</a></th>-->
							<!--<th>Delete</th>-->
		      		...
		      		<tbody>
		        		<tr ng-repeat="clint in clientList">
                            ...
                            <td>{{clint.drop_date}}</td>
							<!--<td >{{clint.fullname}}</td>-->
		          			<td>{{clint.address}}</td>
		          			<td>
                                {{clint.cell_no_1}}
                            </td>
                            <td>
                                <span >{{clint.followup_date}}</span>
                            </td>
                            <td>
                                <span >{{clint.remarks}}</span>
                            </td>
                            <!--  <td>{{clint.zone_name}}</td>-->
		          			<!--<td>{{changeDateFormate(clint.LastTime_login)}}</td>-->
							<td>
								<span >{{clint.sale_raps_name}}</span>
							</td>
                            <td>
                                <svg ng-show="!clint.update" height="30" width="30" style="float: left">
                                    <circle cx="12" cy="12" r="10" stroke="black" stroke-width="1" fill="{{clint.tag_color_code}}"></circle>
                                    Sorry, your browser does not support inline SVG.
                                </svg>
                                <select ng-change="changeColorTag(clint)" ng-show="clint.update" class="form-control" ng-model="clint.tag_color_id">
                                    <option value="0">Select Tag</option>
                                    <option value="{{list.tag_color_id}}" ng-repeat="list in ColorTagList">
                                        <span style="color: red">{{list.tag_color_name}}</span>
                                    </option>
                                </select>
                            </td>
							<td width="150px">
                                <button  title="Edit" type="button" ng-click="getOneClint_marks_list(clint)" class="btn btn-default btn-xs"><i style="margin: 2px" class="glyphicon glyphicon-briefcase"></i></button>

                                <button ng-show="!clint.update" title="Edit" type="button" ng-click="editTag(clint)" class="btn btn-default btn-xs"><i style="margin: 2px" class="fa fa-edit"></i></button>
                                <button ng-show="clint.update" title="Edit" type="button" ng-click="saveTag(clint)" class="btn btn-primary btn-xs"><i style="margin: 2px" class="fa fa-save"></i></button>
		...
		<modal title="" visible="showAddNewClient">
			<form role="form" class="form-group" ng-submit="saveFollowUp_function(remarkObject)">
                <div  class="col-lg-12" ng-show="true" style="background-color: #FFF8DC; margin-top: 10px">
                    <div style="float: left;">
                        <span style="font-weight: bold;">Name :  </span> {{clientObject_detail.fullname}}
                    </div>
                    <div style="float: left;margin-left: 10px">
                        <span style="font-weight: bold;">Address :  </span> {{clientObject_detail.address}}
                    </div>
                    <div style="float: left ; margin-left: 10px">
                        <span style="font-weight: bold;">Contact Number :  </span> {{clientObject_detail.cell_no_1}}
                    </div>
                    <div style="float: left ; margin-left: 10px">
                        <span style="font-weight: bold;">Zone :  </span> {{clientObject_detail.zone_name}}
                    </div>
                </div>

                <div class="col-lg-12 form-group" style="margin-top: 10px">
                   <span style="color: green">{{updatedMessage}}</span>
                </div>

                <div class="col-lg-12 form-group" style="background-color: #FFF0F5"></div>


				<div class="col-lg-12 form-group" style="background-color: honeydew">
					<div class="col-lg-4" style="padding: 10px">
						<span style="font-weight: bold;font-size: 13px; padding: 8px">Follow up Remarks:</span>
					</div>
					<div class="col-lg-8">
						<input type="text"   ng-model="remarkObject.remarks" class="form-control" required/>
				...
				<div class="col-lg-12 form-group" style="background-color: #FFF0F5">
					...
					<div class="col-lg-8">
                        <input  class="form-control"    datetime-picker date-only  date-format="yyyy-MM-dd" type="text" required ng-model="remarkObject.date" size="2">
                ...
                <div ng-show="reasonSelectDiv"  class="col-lg-12 form-group" style="background-color: honeydew">
                    <div class="col-lg-4" style="padding: 10px">
                        <span style="font-weight: bold;font-size: 13px; padding: 8px">Reason:</span>
                    </div>
                    <div class="col-lg-8">
                        <select style="width:80% ;float: left" class="form-control" ng-model="remarkObject.sample_client_drop_reason_id">
                            <option value="0">Select Reason</option>
                            <option  value="{{list.sample_client_drop_reason_id}}" ng-repeat="list in dropReasonList">{{list.reason}}</option>
                        </select>
                        <button style="width:10% ;float: right;margin-top: 10px"  type="button" class="btn-success  btn-sm" ng-click="makeDrop_function()"> Save</button>
                    </div>
                </div>

				<div class=" form-group ">
					<button  ng-disabled="allow_delete =='0'"   class="btn-success  btn-sm">Save Follow up</button>

					<button type="button" ng-disabled="allow_delete =='0'" ng-click="makeRegular_function()"  class="btn-success  btn-sm">Make Regular</button>
					<button ng-disabled="allow_delete =='0'"  type="button" ng-click="drop_cleint_div()" class="btn-success  btn-sm">Drop</button>
            ...
            <table id="customers">
                ...
                <tbody>
                <tr ng-repeat="product in remarkList track by $index">
                    <td>{{$index + 1}}</td>
                    <td>{{product.date}}</td>
