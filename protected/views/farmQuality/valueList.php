...
<script src="<?php echo Yii::app()->theme->baseUrl; ?>/js/viewPageJs/qualityValue/valueList-grid.js"></script>
		...
		<div class="tabbable">
			...
			<div class="tab-content">
				<div class=" active" id="tab_1" style="margin-bottom: 10px; margin: 10px">
					...
					<input  style="float: left ; width: 20% ;" class="form-control input-sm"    datetime-picker date-only  date-format="yyyy-MM-dd" type="text" required ng-model="todate" size="2">
					...
					<table id="customers" style="margin-top: 15px">
						...
						<tr style="background-color: #F0F8FF">
							<th ><a href="#"><span style="margin-right: 50px">Date</span> </a></th>
							<th><a href="#"> #</a></th>

							<th>
								<a href="#"> Farm</a>
							</th>
                            <th ng-repeat="farm in Quality">
                                <a href="#"> {{farm.quality_name}}</a>
                            </th>


						</tr>
						</thead>
						<tbody ng-repeat="main_data in farmList track by $index">
                          <tr ng-repeat="data in main_data.data">
                              <td ng-show="$index==0" rowspan="{{main_data.record}}" style="width: 120px">{{main_data.slect_date}}</td>
							  <td>{{$index+1}}</td>
							   <td style="width: 250px">{{data.fram.farm_name}}</td>
							  <td ng-repeat="quantity in data.value">{{quantity.quantity_value}}</td>
						  </tr>
                         <!-- <tr ng-show="loadData" >
                              <td colspan="2"></td>
                              <td>
