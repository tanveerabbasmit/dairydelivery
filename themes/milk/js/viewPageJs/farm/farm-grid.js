...
app.controller('manageZone', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    ...
    $scope.init = function (allow_delete , zoneList, companyBranchList , saveNewZoneURL ,editZoneURL , deleteURL) {
        ...
        $scope.zoneObject = {'name':'' ,'phone_number':'' , 'companyBranch':'1', 'is_active':'1' ,'is_deleted':'1'}
        $scope.search = '';
        $scope.searchBar = ''
    ...
    $scope.AddQuality = function(){
        $scope.zoneList.push({"farm_id":"","farm_name":"",'phone_number':'' ,"update":true, 'is_active':'1'})
    ...
    $scope.editZoneFunction = function (saveZone) {
        ...
        $http.post($scope.editZoneURL, sendData)
            .success(function (data, status, headers, config) {
                if(data.success){
                    saveZone.update = false ;
                    saveZone.farm_id = data.message ;
                }else{
