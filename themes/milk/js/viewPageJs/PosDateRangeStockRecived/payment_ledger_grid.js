...
riderDailyStockGridModule.controller('riderDailyStockGridCtrl', ['$scope', '$http', '$filter', function ($scope, $http, $filter , $timeout) {
    ...
    $scope.init = function (data,base_url) {
       ...
       //  $scope.select_shop_ledger();
        $scope.get_vendor_list(1);

    }
    $scope.get_vendor_list = function (vendor_type_id){
        $scope.vendor_list_type_wise =  $scope.vendor_list[vendor_type_id];
        $scope.vendor_id = '0';
    }
     ...
     $scope.select_shop_ledger = function(){
         $scope.vendor_id =  document.getElementById("vendor_id_value").value;
         ...
         if($scope.vendor_id =='0' ){
             ...
             var data = {
                 ...
                 'vendor_type_id'  :$scope.vendor_type_id
             ...
             $http.post($scope.base_url+"_get_paymentnew_ledger" ,data)
                 .success(function(responce , status ){
                     $scope.list_data =responce.list;
                     $scope.total_purchase =responce.total_purchase;
                     $scope.total_payment =responce.total_payment;
                     $scope.total_balance =responce.total_balance;
