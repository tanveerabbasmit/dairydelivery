...
app.controller('clintManagemaent', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    ...
    $scope.init = function (data , base_url){
        ...
        try{
            $scope.product_id =   $scope.productList[0].product_id;
    ...
    $scope.getCustomerLedgerReportFunction = function(){
        ...
        if($scope.start_date >$scope.end_date ){
           ...
           var data ={
            ...
            $http.post( $scope.base_url+"_cash_out_detail_list" , data)
                .success(function(responce , data){
                   $scope.list_expence = responce.list_expence;
                   $scope.farm_payment = responce.farm_payment;
                   $scope.vendor_payment = responce.vendor_payment;
                   $scope.total_amount = responce.total_amount;
