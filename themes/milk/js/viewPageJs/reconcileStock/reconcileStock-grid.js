...
riderDailyStockGridModule.controller('riderDailyStockGridCtrl', ['$scope', '$http', '$filter', function ($scope, $http, $filter , $timeout) {
    //INITIALIZATION ===============================================================
    $scope.init = function (lableObject ,productList , todayData , getDialyDeliveryCustomerURL ,googleMapURL , saveDeliveryURL) {
    ...
    $scope.riderData3 = function(todate , check_increment) {
        $scope.todate = todate;
        ...
        var numberOfLoadRider= 0;
            ...
            $http.post($scope.getDialyDeliveryCustomerURL+'2', sendData)
                .success(function (data, status, headers, config) {
                    ...
                    $scope.riderStockListloading.push(data);
                    check_increment++;
                    $scope.loadPerCentage = ((Number(check_increment)/Number($scope.pervousDate_count)))*100;

                       if(check_increment <= $scope.pervousDate_count){

                           $scope.riderData3(todate ,check_increment);
                       }else {
                           check_increment--;
                           $scope.loadPerCentage = ((Number(check_increment)/Number($scope.pervousDate_count)))*100;
                           $scope.showProgressBar = false ;
