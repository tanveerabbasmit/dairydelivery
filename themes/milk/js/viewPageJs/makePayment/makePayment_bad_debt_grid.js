...
app.controller('clintManagemaent', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    //INITIALIZATION ===============================================================
    $scope.init = function (base_url,required_name ,collectionvault_list , date_object ,disabled_result ,discount_type ,allow_delete ,Currentyear ,todayMonth , company_id ,clientList , paymentMethod , oneCustomerAmontListURL , checkAccountBalnceUrl){
    ...
    $scope.save_bad_payment =function (){
    ...
    $scope.deleteClientButton = function (payment_master_id) {
        ...
        if (person=="32147") {
            ...
            $http.post( $scope.base_url+"_delete_bad_payment", payment_master_id)
    ...
    $scope.get_today_payment_list = function () {
        ...
        $http.post($scope.base_url+"_bad_debt_payment" ,$scope.paymentObject.client_id)
            .success(function(responce){
   ...
   $scope.abcd = function (y) {
    ...
    $scope.getOneCustomerPaymentList = function (client_id) {
        $http.post($scope.base_url+"_bad_debt_payment" ,client_id )
            .success(function(responce){
    ...
    $scope.update_payment = function(list) {
        ...
        $http.post($scope.base_url+"_bad_debt_update" ,list )
            .success(function(responce){
                if(responce){
                    $scope.getOneCustomerPaymentList($scope.paymentObject.client_id);
                }else {
