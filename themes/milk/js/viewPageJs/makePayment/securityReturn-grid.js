...
app.controller('clintManagemaent', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
    ...
    $scope.init = function (discount_type ,allow_delete ,Currentyear ,todayMonth , company_id ,clientList , paymentMethod , oneCustomerAmontListURL , checkAccountBalnceUrl){
        ...
        $scope.paymentObject = {'bill_year': $scope.selectYear , 'bill_month': $scope.selectMonth, 'amount_paid':'','trans_ref_no':'','client_id':'','remarks':'2' ,'company_branch_id':company_id,'payment_mode':'3','startDate':$scope.startDate};
   ...
   $scope.savePaymernt = function() {
      ...
      if($scope.paymentObject.client_id==''){
          ...
          var send_data ={
             ...
             'security_amount':$scope.security_amount,
             'client_id':$scope.client_id,
          }
          $http.post($scope.paymentMethod+'return_security_save' ,send_data)
              .success(function(responce){
    ...
    $scope.getOneCustomerPaymentList = function (client_id) {
        ...
        // alert($scope.oneCustomerAmontListURL);
        $http.post($scope.oneCustomerAmontListURL+"_security_amount" ,client_id )
            .success(function(responce){
               $scope.security_amount = Number(responce);
