...
riderDailyStockGridModule.controller('riderDailyStockGridCtrl', ['$scope', '$http', '$filter', function ($scope, $http, $filter , $timeout) {
    //INITIALIZATION ===============================================================
    $scope.init = function ( fiveDayAgo ,  company_id , riderList , getDialyDeliveryCustomerURL ,googleMapURL , saveDeliveryURL) {
       ...
       $scope.endDate =  $scope.todate;
        ...
        $scope.paid_month ='date';


        $scope.resultList =[];
        $scope.searchproduct ='';
        $scope.select_all_cattle = true;
        $scope.productinData();
        $scope.productinData_drop_down();


    }

     $scope.productinData_drop_down = function () {
          ...
          var sendData ={
              ...
              select_all_cattle :  $scope.select_all_cattle,
         ...
         $http.post($scope.getDialyDeliveryCustomerURL, sendData)
             .success(function (data, status, headers, config) {
                 $scope.imageLoading = false ;
                 $scope.resultList_drop_down =data.resultList;
                 $scope.select_un_select_resultList_function();
     ...
     $scope.productinData = function () {
          $scope.imageLoading = true ;
          var sendData ={
              startDate : $scope.startDate,
              endDate : $scope.endDate,
              select_all_cattle :  $scope.select_all_cattle,
              resultList :  $scope.resultList_drop_down,
          }
         $http.post($scope.getDialyDeliveryCustomerURL, sendData)
             .success(function (data, status, headers, config) {
     ...
     $scope.select_un_select_resultList_function_2 = function (){
        return false;
     }
     $scope.select_un_select_resultList_function =function(){

         angular.forEach( $scope.resultList_drop_down , function (value,key){
             value.selected = $scope.select_all_cattle;
         });
    ...
    $scope.click_on_cattle = function (){
        $scope.select_all_cattle = false;
    }
